...
def get_feiertage(self, sachb, datum, anz_tage):
    ...
    sql_str ="""select f.datum, f.faktor from feiertage f, sachbearbeiter s where s.sachb = '%(sachb)s' and  s.fkid = f.fkid and f.datum > date '%(datum)s' - interval '%(anz_tage)s days' and f.datum < '%(datum)s' order by f.datum
            """ % vars_dict
