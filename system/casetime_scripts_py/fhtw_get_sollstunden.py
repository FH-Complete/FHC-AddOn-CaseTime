...
def get_sollstunden(self, datum):
    ...
    sql_str = """select sachb, datum::date, getzeitmodellsollstunden(zeitmodell, datum::date, 1) as sollstunden, get_anwesenheitszeit(sachb, datum, 'N') from public.zeitrohdaten where datum like '%(datum)s%%' group by sachb, datum, zeitmodell order by datum asc""" % vars_dict
