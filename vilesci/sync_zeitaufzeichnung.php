...
if($result = $db->db_query($qry))
{
	...
	while($row = $db->db_fetch_object($result))
	{
		...
		if ($row->aktivitaet_kurzbz == 'LehreExtern')
		{
			...
			$qry_tag = "select numberOfElements(*) from addon.tbl_casetime_zeitaufzeichnung where uid = '".$row->uid."' and datum = '".$row->datum."' and zeit_start < '".$row->startzeit."' and zeit_ende > '".$row->endzeit."' and typ = 'ko'";
